...
public class SaveAfterActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        int type = intent.getIntExtra("type", 0); ////0 从旅途中进入 1从记录界面进入  2从记录详情进入
        share_url = Constants.URL_PHOTOSSHARE + "?pathInfoId=" + id;
        if (type == 1) {
            ...
            Glide.with(this).load(model.getPicUrl()).into(bgImg);
            ...
            url=bean.getPicurl();
            Glide.with(this).load(url).into(bgImg);
            ...
            Glide.with(this).load(url).into(bgImg);
            nameTxt.setText(StringUtils.isStringNull(bean.getPathname())?bean.getAddress():bean.getPathname());
            share_title=StringUtils.isStringNull(bean.getPathname())?"":bean.getPathname();
            share_content=StringUtils.isStringNull(bean.getPathname())?bean.getAddress():bean.getPathname();
            String time = DataUtils.format(bean.getCreatetime(), "yyyy-MM-dd HH:mm");
            timeTxt.setText(time);
        }
    ...
    private UMShareListener umShareListener = new UMShareListener() {
        ...
        public void onResult(SHARE_MEDIA platform) {
            ...
            if (platform.name().equals("WEIXIN_FAVORITE")) {
                Toast.makeText(SaveAfterActivity.this, platform + " 收藏成功啦", Toast.LENGTH_SHORT).show();
            } else {
                Toast.makeText(SaveAfterActivity.this, platform + " 分享成功啦", Toast.LENGTH_SHORT).show();
            }
        }

        @Override
        public void onError(SHARE_MEDIA platform, Throwable t) {
            Toast.makeText(SaveAfterActivity.this, platform + " 分享失败啦", Toast.LENGTH_SHORT).show();
            if (t != null) {
        ...
        public void onCancel(SHARE_MEDIA platform) {
            Toast.makeText(SaveAfterActivity.this, platform + " 分享取消了", Toast.LENGTH_SHORT).show();
        }
    };
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.update_txt:  //编辑
                shareLin.setVisibility(View.GONE);
                startActivity(new Intent(this,AddImgMakerActivity.class).putExtra("id",id).putExtra("url",url).putExtra("type",1));
                break;
        ...
        RemoteDataHandler.asyncTokenPost(url, this, false, params, new RemoteDataHandler.Callback() {
            ...
            public void dataLoaded(ResponseData data) throws JSONException {
                ...
                if (status.equals("1")) {
                    ...
                    ToastUtils.show(SaveAfterActivity.this, message);
                }
            }
        });
    }
